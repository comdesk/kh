...
-- ------------------------------------------------------
-- 5. HAVING 조건식 (2차 필터링 - GROUP BY가 만들어낸 그룹을 필터링)
-- ------------------------------------------------------
-- GROUP BY 절에 의해 생성된 결과(그룹들) 중에서, 
-- 지정된 조건에 일치하는 데이터를 추출할 때 사용
--  
...
GROUP BY
    department_id                   -- 2nd.
HAVING
    sum(salary) >= 90000            -- 3rd. (GROUP BY절 다음에 생성된 그룹에 대해선 집계함수 사용 가능)
ORDER BY
    1 ASC;                          -- 5th.
...
SELECT
    department_id,
    -- count(department_id) AS CNT1,      -- 유니크x, 결측치o
    count(employee_id) AS CNT2
FROM
    employees
GROUP BY
    department_id                      -- NULL 그룹도 생성됨을 기억할 것!!!
-- HAVING
--     -- count(salary) >= 6              -- 2nd. filtering (for groups). (직원 수가 6명 이상)
--     count(employee_id) >= 6            -- 유니크하고, 결측치 없는 사번으로 직원 수를 구하는 게 가장 안전
-- HAVING
--     salary >= 3000                  -- XX: 각 그룹에 대해, 단순컬럼들만 사용불가 (그룹에 대한 특성이 아닌 개별 사원에 대한 급여는 사용 불가)
-- HAVING
