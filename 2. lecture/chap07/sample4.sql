...
-- 4. MERGE 문
-- ------------------------------------------------------
--  가. 스키마(구조)가 같은 두 개의 테이블의 데이터를 비교하여,
--  나. 하나의 테이블로 합침. (CUD 작업이 동시에 일어남.)
...
--
--      --------------------------------------
--      (1) 조인 조건식이 일치(참이면)하면... (같은 행이라는 뜻)
--      --------------------------------------
--      WHEN MATCHED THEN
--          --------------------------------------
--          -- (1-1). MERGE 테이블의 기존 데이터로 변경(갱신).
--          --------------------------------------
--          UPDATE SET
--              컬럼명1 = 값1,
--              ...
...
--
--      --------------------------------------
--      (2) 조인 조건식이 비일치(거짓이면)하면 ... (신규행. 한쪽에 없는 행)
--      --------------------------------------
--      WHEN NOT MATCHED THEN
--          --------------------------------------
--          -- (2-1). MERGE 테이블에 새로운 데이터 생성.
--          --------------------------------------
--          INSERT (컬럼목록)       -- INTO는 이미 MERGE문에 있음.
--          VALUES (값목록)
--          [WHERE 조건식];
-- 
-- ------------------------------------------------------
...
CREATE TABLE p_total (      -- 머지된 결과 저장할 테이블. 스키마 동일해야 함.
    판매번호 VARCHAR2(8),
    제품번호 NUMBER,
    수량 NUMBER,
    금액 NUMBER
