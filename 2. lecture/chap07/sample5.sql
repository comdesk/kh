...
-- ------------------------------------------------------
--  가. Transaction (트랜잭션):
--      a. DB의 논리적인 작업단위 => 전체가 성공하든가, 실패하든가
--      b. 분리될 수 없는, 한 개 이상의 DB조작을 의미
--      c. 하나의 트랜잭션에는, 한 개 이상의 SQL문장이 포함가능
--      d. 트랜잭션 대상 SQL문장은, "DML" 문임(****) => transaction unit
...
--            (Partial Rollback)
--  라. 락 경합(Race Condition)으로 인한, 데드락(Dead Lock) 발생가능 (멀티쓰레딩 관련)
...
--  무한 대기 수행(Infinite awaiting) --> 락 경합(Lock Contention = Race Condition)
-- ----------------------
delete from dept
where
    deptno = 40;