...
--
--      * Oracle은 날짜정보를 내부적으로 7바이트 숫자로 관리 -> 산술연산가능 (틱시그널)
--  (4) 변환 (처리)함수
--  (5) 일반 (처리)함수
--
...
SELECT
    -- sysdate,             --잊어버려라!! (Local DB이든, Cloud DB이든)
    -- systimestamp,        --잊어버려라!! (Local DB이든, Cloud DB이든)
    current_date,
    current_timestamp
FROM
    dual;
...
ALTER SESSION SET NLS_DATE_FORMAT = 'DD-MON_RR';     -- VSCODE format
...
SELECT
    ...
    current_date + 1 AS 내일,     -- 현재날짜 + 하루  (2)
    -- current_date * 3,          -- XX
    -- current_date / 3,          -- XX
    current_date - 1 AS 어제,     -- 현재날짜 - 하루  (3)
    current_date - (current_date - 1), -- 차이가 발생한 "일수"  (4)
    current_date + 3/24 AS "3시간 후",      --(5)
    current_date + 3/24/60 AS "3분 후",     --(6)
    current_date + 3/24/60/60 AS "3초 후"   --(7)
FROM
    dual;
...
-- 음수값을 지정하면, 현재 날짜에 지정된 개월 수만큼 뺀 날짜를 반환
-- ------------------------------------------------------
SELECT
    current_date AS 오늘,
...
SELECT
    ...
    next_day(add_months(hire_date, 5), 1)
...
-- ------------------------------------------------------
-- ROUND(date1, 'YEAR') : 지정된 날짜의 년도를 반올림(to YYYY/01/01) (그 해가 절반 이상 지났는가)
-- ROUND(date1, 'MONTH'): 지정된 날짜의 월을 반올림(to YYYY/MM/01)  (그 달이 절반 이상 지났는가)
-- ------------------------------------------------------
SELECT
    last_name,
    hire_date,
...
-- (7) 날짜 (처리)함수 - TRUNC
-- ------------------------------------------------------
-- 날짜를 가장 가까운 년도 또는 월로 절삭하여 반환 (두 날짜가 모두 1/1이면 계산할 때 소숫점 안 나옴)
-- ------------------------------------------------------
-- TRUNC(date1, 'YEAR') : 지정된 날짜의 년도를 절삭(to YYYY/01/01)
-- TRUNC(date1, 'MONTH'): 지정된 날짜의 월을 절삭(to YYYY/MM/01)
-- ------------------------------------------------------
SELECT
    last_name,
    hire_date,
