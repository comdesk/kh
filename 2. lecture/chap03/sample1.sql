...
--                    (단, 문자열의 중간은 처리못함)  공백문자를 주로 제거할 뿐, 공백만 삭제하는 것이 아님.
--  (2) 숫자 (처리)함수 : 
--  (3) 날짜 (처리)함수
--  (4) 변환 (처리)함수
--  (5) 일반 (처리)함수
...
SELECT
   -- || (Concatenation Operator) == concat function
   'Oracle' || 'Sql' || 'third',    --***
   concat( concat('Oracle', 'Sql'), 'third')    -- 함수의 중첩
FROM
   dual;
...
--     문자열의 길이 반환
-- ------------------------------------------------------
--  A. LENGTH   returns Characters  문자의 길이
--  B. LENGTHB  returns Bytes 문자가 몇 바이트인지
-- ------------------------------------------------------
SELECT
   'Oracle',
   length('Oracle'),
   lengthb('Oracle')    -- 디비 서버에서의 바이트 수
FROM
   dual;
...
SELECT
    unistr('\D55C\AE00'),     --유니코드를 문자로 표현해주는 함수
...
-- ------------------------------------------------------
SELECT
   instr('MILLER', 'L', 1, 2),   -- 1: offset(몇 번째 인덱스부터 찾겠느냐), 2: occurence(중복됐을 때 몇 번째 발생을 원하는가)
...
SELECT
   ...
   -- substr('123456-1234567', 8),  --offset부터 끝까지
...
SELECT
   hire_date,
   to_char(hire_date) AS 입사일1,    -- 위와 결과는 동일하지만, 문자열 타입.
   to_char(hire_date, 'RR/MM/DD') AS 입사일2,
   to_char(hire_date, 'yyyy/MM/dd - hh24:mi:ss') AS 입사일3,
   substr( to_char(hire_date), 8, 2 ) AS 입사년도  -- -2로 하면 편함.
FROM
   employees;
...
-- ------------------------------------------------------
-- (9) 문자(처리) 함수 - LPAD    보고서 만들 때 이용
-- ------------------------------------------------------
--      문자열 오른쪽 정렬 후, 
--      왼쪽의 빈 공간에 지정문자 채우기(padding)
-- ------------------------------------------------------
SELECT
   lpad('MILLER', 10, '*')  --두 번째 인자가 전체 문자 길이
FROM
   dual;
...
-- ------------------------------------------------------
SELECT
   ltrim('MMMIMLLER', 'M')    -- 더 이상 삭제할 문자가 없을 때 멈춤.
FROM
   dual;


SELECT
   ltrim(' MILLER '),   -- 삭제할 문자 지정하지 않으면 디폴트로 왼쪽 공백 삭제
   length( ltrim(' MILLER ') )
FROM
   dual;
...
-- ------------------------------------------------------
-- 문법)
--    TRIM( LEADING 'str' FROM 컬럼명|표현식 )     앞에서부터 지워라
--    TRIM( TRAILING 'str' FROM 컬럼명|표현식 )    뒤에서부터 지워라
--    TRIM( BOTH 'str' FROM 컬럼명|표현식 )        양쪽에서 지워라
