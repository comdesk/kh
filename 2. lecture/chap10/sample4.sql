...
--  b. 특정 컬럼값의 범위가 넓을 경우.
--  c. WHERE절에 사용되는 컬럼
--  d. Join 조건에 사용되는 컬럼(FK에 대해서, PK는 이미 있음)
--  e. DQL 쿼리결과의 크기가, 전체 데이터의 2 ~ 4 % 이내를 검색하는 경우(너무 많은 행을 뽑아내면 걍 전체 검색이 나음)
...
-- 사유) 인덱스가 걸린 컬럼에 대해서, 함수로 가공처리 (오류가 아니라 그냥 인덱스 안 쓰고 풀스캔함.)
SELECT
  *
FROM
  emp
WHERE
  to_number(empno) = 7369;    -- empno는 pk
