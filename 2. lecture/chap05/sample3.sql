...
-- 다. Check조건을 WHERE 절에 기재(조인조건과 Check조건을 분리)
-- 라. 가독성 향상
-- ------------------------------------------------------
...
--         with implicit(모호한) columns automatically searched.
--  c. USING(Common Columns) or ON <Join Condition>     Using절, On절
--      == The same as Oracle Equal Join
...
FROM
    ...
    -- employees, departments; in Oracle Cartesia Product 집합 연산
...
SELECT
    ...
    -- employees.last_name,    -- XX: ORA-00904: "EMPLOYEES"."LAST_NAME": invalid identifier
...
SELECT
    ...
    department_name
FROM
    -- 자연조인(Natural Join) == 동등조인(Equi Join) + 중복컬럼제거
    -- 자연조인은 성질을 보지 못하고, 이름만 같으면 이어 버림.
    -- 잘못된 조인을 하게 됨.
...
SELECT
    ...
    manager_id              -- OK: 두 테이블의 공통컬럼 기재시, 테이블 별칭은 제거해야 함(*** 오라클과 반대)
                            -- (중복된 컬럼을 제거했기 때문에 컬럼의 모호성이 제거돼서(왼쪽 중복을 제거할지, 오른쪽 중복을 제거할지 모르니까))
    -- ORA-25155: column used in NATURAL join cannot have qualifier(한정어, 수식어)
...
--    실행된다. (공통컬럼의 이름들만 쉼표찍고 나열할 수만 있기 때문에)
-- 라. ON <Join condition>:
--    Non-equal Join 이나, 임의의 조건으로 Join 할 경우에 사용, Check 조건도 기재 가능
-- ------------------------------------------------------
...
SELECT
    ...
    -- departments.department_id    -- XX: 공통컬럼은 수식어(Qualifier)를 가져서는 안된다.
...
FROM
    ...
    USING(department_id);   -- 무조건 지정된 공통컬럼으로 동등조인 수행
...
FROM
    ...
    -- USING절의 한계: 무조건 Equi Join -> Non-Equi Join 불가
    USING(department_id, manager_id)    -- => 위의 내추럴 조인과 동일
    -- 공통컬럼이 한 개 이상이라면 쉼표 찍고 나열 
WHERE
    department_id = 90;  -- 검색조건
...
FROM
    ...
    -- employees t1 JOIN departments t2            -- INNER 키워드 생략가능
    ON t1.department_id = t2.department_id      -- 조인조건
    AND t1.department_id = 90;                  -- Check조건 (가독성 저하)
    -- WHERE 절이 없어짐.
...
FROM
    ...
    -- 위의 JOIN된 두 테이블의 결과셋이 세 번째 테이블과 JOIN할 결과셋이 됨.
    -- => 조인의 순서에 따라 결과가 달라질 수 있음. (이건 결과 같음)
...
SELECT
    ...
    d.department_name AS 부서명,
    g.grade_level AS 등급,
    department_id AS 부서번호         -- OK: using절과 on절 혼용하면 using절의 영향
    -- e.department_id AS 부서번호    -- XX: ORA-25154: column part of USING clause cannot have qualifier
FROM
    -- employees e INNER JOIN departments d
    employees e JOIN departments d              -- INNER 키워드 생략가능
...
FROM
    ...
    -- employees e LEFT OUTER JOIN employees m        -- 왼쪽 테이블 살리기
    -- employees e LEFT JOIN employees m           -- OUTER 키워드 생략가능

    -- employees e RIGHT OUTER JOIN employees m    -- 오른쪽 테이블 살리기
