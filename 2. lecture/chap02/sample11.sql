...
--  FROM 테이블명
--  [ WHERE 조건식 ]
--  [ ORDER BY { column|표현식 } [ASC|DESC] ];  ASC,DESC가 생략되면 기본은 ASC
-- ------------------------------------------------------
...
ORDER BY   
   email DESC;      --SELET 절에 없는 컬럼으로 정렬            

ORDER BY
    salary + 100 DESC;  -- 표현식(SELECT절에서 만든 표현식 (SELECT 절에 없는 표현식도 가능))으로 정렬


-- ORDER BY 절에 컬럼인덱스 사용 (SELECT 절에 기재된 순서대로 인덱스 생성)
...
ORDER BY
    4 DESC;    -- 컬럼인덱스로 정렬
-- 가능하면 쓰지 않는 게 좋음(SELECT 절에 나열된 컬럼은 변동될 수 있음)
