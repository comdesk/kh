...
FROM
    employees;
-- Null의 가능성이 있는 commission_pct를 연산식에 넣으면 연봉이 사라짐.
...
--                  매우 주의해서 처리함
--
--    결측치 처리 예: 대표값(= 특성값) - 평균/중위수/최빈값/합계/제거!/보간(interpolation)
-- ------------------------------------------------------
-- * Caution: To use NVL(column, defaultValue) built-in function (널 값 포함 column)
-- ------------------------------------------------------
SELECT
    last_name 이름,
    salary 월급,
