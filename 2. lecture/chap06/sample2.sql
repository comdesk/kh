...
--      - 기본키(Primary Key)를 이용하는 경우   => 기본키가 유니크하게 단 한 개의 행을 구별하기 때문에
--      - 그룹함수(MAX, MIN, SUM 등)를 이용하는 경우 => 집계함수는 값을 단 한 개 반환하기 때문에
-- ------------------------------------------------------
...
-- ------------------------------------------------------
-- 평균 급여(기준정보)를 먼저 구하기 위해, 단일 행 서브쿼리 및 그룹함수 AVG 사용
-- 따라서, 메인쿼리에서 사용가능한 연산자는 하나의 값과 비교하는 비교연산자임!

SELECT
    last_name,
    salary,
    ( SELECT avg(salary) FROM employees ) AS 평균급여   -- 단일행비상관부속질의
    -- 정말 salary가 월 평균급여를 넘는지 눈으로 보려고
FROM
    employees
WHERE
    ...
    salary >= ( 
        -- "비상관" 서브쿼리(=부속질의)란? (***)
        -- (1) 메인쿼리의 도움 없이(데이터를 제공받지 않고), 독립적으로 실행 가능한 부속질의
        -- (2) 특징: 항상 메인쿼리보다 먼저 수행된다.
        -- (3) 먼저 수행되어 나온 결과셋을 바깥쪽 메인 쿼리에 전달하는 목적

        -- 단일 행 "비상관" 서브쿼리: 모든 사원의 평균 월급여 반환
        -- 메인쿼리로 결과값 전달
        SELECT
            avg(salary) AS AVG1   -- => 반드시 Scalar(1x1)로 반환해야 비교연산자 작동 가능
            -- avg(salary) AS AVG2  => 1x2면 avg1랑 비교할지 avg2 비교할지 모르게 됨.
        FROM
            employees
    );
...
-- ------------------------------------------------------
-- (2) 100번 부서의 최대 월급여(기준 정보)와 동일한 월급을 받는 사원조회
-- ------------------------------------------------------
SELECT
    last_name,
    salary
