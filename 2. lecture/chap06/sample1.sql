...
--  다. 한 개 이상의 SELECT문장을, 하나로 합쳐서, 하나의 실행가능한 
--      문장으로 만들어, 원하는 데이터를 조회하는 방법
--  라. 종류:
--      (1) 상관 부속질의
...
--  FROM 테이블
--  WHERE 컬럼명 연산자 (
--      SELECT select_list FROM 테이블    -- 서브쿼리(안쪽 SELECT 문장. 소괄호도 포함)
--  );  -- 서브쿼리는 소괄호 필수
-- ------------------------------------------------------
...
FROM
    employees
WHERE
    salary >= 4400;   -- Whalen의 월급지정 (이 값을 알아야 답이 가능)
