...
-- 사용하는 SQL문장.
-- ------------------------------------------------------
-- 가. 자동으로 COMMIT 됨. (즉, DB에 자동 영구 반영됨) (autocommit으로 조절 못함.)
...
--  (5) 동의어(Synonym) - 테이블의 별칭 (테이블이름과 동일한 목적으로 쓰이기 때문에 동의어)
...
-- ORA-00942: table or view does not exist (hr계정으로 접속해서 실행하면 접근권한이 없어서 안 보임.)
SELECT
    deptno,
    dname,
    loc
...
--      b. 입력할 데이터 크기가 유동적 -> 공간(하드디스크 공간)낭비 초래(공백으로 낭비)
...
-- 테이블 내의 특정 행을, ROWID 값으로, 가장 빠르게 조회가능
...
FROM
    dept
WHERE
    ROWID = 'AAAR/0AAMAAAACHAAB'; -- ROWID 이용한 조건 검색

-- ------------------------------------------------------

CREATE TABLE employee (   -- 스키마.객체명(scott.employee) 사용가능
    empno NUMBER(4),    -- 정수 형태
    ename VARCHAR2(20),
    hiredate DATE,
    sal NUMBER(7,2)     -- 실수 형태(소수점자리까지 포함해서 7자리)
);      -- employee 테이블 생성
...
--  * Default 옵션
-- ------------------------------------------------------
-- 가. 테이블에 데이터 저장 시(INSERT), 특정 컬럼에 값을 지정하지 않으면,
--     자동으로 NULL 값이 저장됨 (묵시적)
-- 나. 컬럼에 값을 지정하지 않아도, 자동으로 기본값 입력
-- 다. NULL 값이 입력 방지
-- 라. 주로 현재 날짜/성별 같은, 고정된 값만 가지는 컬럼에 대해 사용
-- 마. DESC로 보는 테이블 명세에는 나오지 않음.
-- ------------------------------------------------------
