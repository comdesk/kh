...
--  마. 데이터가 저장되기 전에, 제약조건에 맞게 무결성을 검사하여,
--      잘못된 데이터가 저장되는 것을 방지
--  바. 테이블에 행이 삽입(INSERT)/수정(UPDATE)/삭제(DELETE)
--      될 때마다 적용
--  사. 필요 시, 제약조건의 기능을 일시적으로, 
--      활성화(Enable)/비활성화(Disable) 가능 (보통 더미 데이터를 한꺼번에 들이부을 때 비활성화)
--  아. USER_CONSTRAINTS 데이터 딕셔너리에 저장
--  자. 제약조건의 종류: (한 개 이상의 컬럼에 제약조건 가능)
--      (1) NOT NULL    - Only ( column-level ): 컬럼 오른쪽에 선언
--          a. 해당 컬럼값으로 NULL 허용하지 않는다 (UPDATE를 NULL로 해도 허용X)
--      (2) UNIQUE      - Both ( column-level ) and ( table-level )
--          a. 해당 컬럼값은 항상 유일한 값을 갖는다 (모든 행에 대해서 컬럼값이 유일해야 한다.)
--          b. ( NULL 허용: NULL은 비교 자체가 안 되니, NULL도 고유한 값으로 본다. )
--          c. UNIQUE 성질로 인하여, 자동으로 UNIQUE INDEX 생성됨.
--      (3) PRIMARY KEY - Both ( column-level ) and ( table-level )
--          a. 해당 컬럼값은 반드시 존재해야 하고 유일해야 한다
--          b. ( NOT NULL + UNIQUE )
--          c. 해당 테이블에서 각 행들을 유일하게 구분해주는 식별기능
--          d. 테이블 당, 반드시 하나만 가질 수 있음 (한 개의 컬럼이라는 뜻 아님.)
--          e. 단순/복합 컬럼으로 구성 가능
--          f. UNIQUE 성질로 인하여, 자동으로 UNIQUE INDEX 생성됨
--          g. 이 기본키를 이용한 데이터 검색은, 자동생성된 인덱스로 인하여
--             속도가 매우 빠르다
--      (4) FOREIGN KEY - Both ( column-level ) and ( table-level )
--          a. 해당 컬럼의 값이, 다른 테이블(부모)의 컬럼의 값을 참조해야 한다
--          b. 참조되는 컬럼(PK)에 없는 값은 저장이 불가능하다
--          c. ( ** NULL 허용 ** )
...
--      d. Oracle의 제약조건명 형식: Prefix(SYS_) + 자동이름(c네자리수)
-- ------------------------------------------------------
...
--  나. 하나 이상의 컬럼에 여러 개의 제약조건을 부여할 경우에 사용
-- ------------------------------------------------------
-- Basic Syntax:
--
--  CREATE TABLE [스키마].테이블명 (
