...
-- 
--      (2) 객체 권한 (Object Privileges) (예: 테이블,인덱스...)
...
--      몇 가지의 중요한 시스템 권한을 해당 사용자에게 할당해줘야 함
-- ------------------------------------------------------
-- Basic syntax:
--  
--  GRANT 시스템권한1 [, 시스템권한2, ...]
...
-- user01 일반계정에,
--  CREATE SESSION (DB접속 시스템권한), (XX)
--  CREATE TABLE (table 생성 시스템권한) (XX)
-- 아래의 3가지 역할 및 권한을 부여(by SYS계정)
--  (1) CONNECT ROLE (역할: 권한들의 모음/집합)
--  (2) RESOURCE ROLE (역할)
--  (3) UNLIMITED TABLESPACE 시스템 권한 (12C 버전부터)
-- 할당(부여)
GRANT
    -- CREATE SESSION,
    -- CREATE TABLE
