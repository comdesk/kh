...
--
--  CREATE USER 사용자계정
--  IDENTIFIED BY 비밀번호;     비밀번호에 문자가 들어가면 ''로.
-- ------------------------------------------------------
...
CREATE USER user01
IDENTIFIED BY oracle;            -- XX : 암호생성규칙미달(로컬에서는 가능)
-- IDENTIFIED BY Oracle12345678;         -- OK : 암호생성규칙준수(클라우드에서)
...
-- {VSC | Oracle SQL*Developer | Oracle SQL*PLUS} 통해 접속시도     -- XX : 접속실패(접속권한조차 없음)
